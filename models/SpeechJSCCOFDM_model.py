...
class SpeechJSCCOFDMModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netE = networks.define_Speech_E(opt, init_type='kaiming',
                                            norm=opt.norm_EG,
                                      init_gain=0.00002, gpu_ids=self.gpu_ids)

        self.netG = networks.define_Speech_G(opt, init_type='kaiming',
                                            norm=opt.norm_EG,
                                      init_gain=0.00002, gpu_ids=self.gpu_ids)
    ...
    def set_blackbox_perturbation(self, wireless_perturbation):
        ...
        total_elements = N * self.opt.P * self.opt.speech_S * self.opt.M
    ...
    def padd_tensor(self, x, N):
        ...
        total_elements = self.opt.P * self.opt.speech_S * self.opt.M
        padd_num = 48 - (total_elements % 48)
        if (total_elements % 48) != 0:
    ...
    def forward(self, inference=False, cof_in=None):

        N = self.real_A.shape[0]

        # Pass the image through the image encoder
        # [batch, channel, w, h] = [1, 12, 8, 8]
        latent, netE_mean, netE_std = self.netE(self.real_A)
        ...
        self.tx = latent.view(N, self.opt.P, self.opt.speech_S, 2, self.opt.M).permute(0,1,2,4,3)
        ...
        # TX reference frame
        tx_ref = latent.view(N, self.opt.P, self.opt.speech_S, 2, self.opt.M).permute(0,1,2,4,3)
        ...
        if padd_num != 0:
            rx2 = rx2[:, :, :-padd_num, :]
        rx2 = rx2.view(N, self.opt.P, self.opt.speech_S, 64, 2)
        ...
        self.fake = self.netG(dec_in, netE_mean, netE_std)
        
        self.MSE_speech = self.criterionL2(self.fake, self.real_B)            
    ...
    def backward_P(self):
        self.forward()
        self.criterionL2 = torch.nn.MSELoss()
        self.loss_P = -self.criterionL2(self.fake, self.real_B) #* self.opt.lam_L1
