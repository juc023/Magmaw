...
def parse_args():
    ...
    parser.add_argument("--wav_path", type=str, default="data/clean_trainset_28spk_wav_8khz",
                        help="path of wavset")
...
if __name__ == "__main__":
    ...
    def wav_processing(wav_file, tfrecords_file, window_size):
        ...
        return num_slices, store_slices
    ...
    for file_count, trainset_wav_file in enumerate(trainset_wav_files):
        ...
        sys.stdout.flush()
        num_slices, output_wave = wav_processing(trainset_wav_file, trainset_tfrecords_file, window_size)
        total_trainset_slices += num_slices
        
        if (len(output_wave) != 0):
            for i in range(len(output_wave)):
                store_data.append(output_wave[i-1])
    ...
    # store in pickle
    assert(len(store_data) == total_trainset_slices)
    
    with open('train_data.pickle', 'wb') as f:
        pickle.dump(store_data, f, protocol=4)#pickle.HIGHEST_PROTOCOL)
    ...
    for file_count, validset_wav_file in enumerate(validset_wav_files):
        ...
        sys.stdout.flush()
        num_slices, output_wave = wav_processing(validset_wav_file, validset_tfrecords_file, window_size)
    ...
    # store in pickle
    assert(len(store_data) == total_validset_slices)
